...
│      ├── ✅ 승인 → 🎉 완성!                                 │
│      │                                                      │
│      └── ❌ 반려 → @implementer (수정 후 재리뷰)             │
│              ↑______________|                               │
│                                                             │
│  💡 Cyclic Feedback Loop:                                   │
│     리뷰에서 이슈 발견 시 구현 단계로 돌아가                 │
│     수정 → 재리뷰 사이클 반복                               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
실제 개발 프로세스처럼, 코드 리뷰에서 이슈가 발견되면:

1. **@code-reviewer**가 문제점과 수정 사항을 정리
2. **@implementer**에게 재전달하여 수정 요청
3. 수정 완료 후 **@code-reviewer**가 재검토
4. 모든 이슈가 해결될 때까지 반복
