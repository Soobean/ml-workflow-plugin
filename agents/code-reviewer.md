...
## 피드백 루프 (Cyclic Review)

심각한 이슈(🔴 High) 또는 다수의 이슈(🟡 Medium 3개 이상) 발견 시:
...
다음 이슈들을 수정한 후 재리뷰가 필요합니다:

| 우선순위 | 파일:라인 | 이슈 | 수정 방법 |
|---------|-----------|------|----------|
| 1 | `src/api.py:45` | 타입 힌트 누락 | `def func(x: int) -> str:` |
| 2 | `src/api.py:67` | 예외 처리 없음 | try-except 추가 |

### 다음 단계

👉 **@implementer**를 호출하여 위 수정 사항을 반영해주세요.
   수정 완료 후 다시 **@code-reviewer**를 호출하여 재검토합니다.

예시:
\`\`\`
@implementer 위 코드 리뷰 결과를 반영해서 수정해줘
\`\`\`
```
